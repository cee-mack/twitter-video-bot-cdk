...
def tweet_object_with_media():
    with open(os.path.abspath("app/tests/resources/object_with_media.json")) as f:
        return json.loads(f.read())


@pytest.fixture
def tweet_object_without_media():
    with open(os.path.abspath("app/tests/resources/object_without_media.json")) as f:
        return json.loads(f.read())
...
def test_return_highest_bitrate(tweet_object_with_media):
    ...
    assert utils.return_highest_bitrate(tweet_object_with_media) == highest_bitrate_url


def test_return_highest_bitrate_with_no_media(tweet_object_without_media):
    assert utils.return_highest_bitrate(tweet_object_without_media) is None
